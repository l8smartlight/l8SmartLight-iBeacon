...
- (int) findBLEPeripherals:(int) timeout
{
    ...
    if([[NSUserDefaults standardUserDefaults] objectForKey:@"device_list"]){
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
            ...
            NSArray* deviceList = [[NSUserDefaults standardUserDefaults] objectForKey:@"device_list"];
            NSMutableArray* uuidList =[NSMutableArray array];
            for (NSString *deviceId in deviceList) {
                [uuidList addObject:[[NSUUID alloc]initWithUUIDString:deviceId]];
            }
            ...
            NSArray *peripheralsNew = [self.CM retrievePeripheralsWithIdentifiers:uuidList];
...
- (void) connectPeripheral:(CBPeripheral *)peripheral
{
    ...
    if(![[NSUserDefaults standardUserDefaults] objectForKey:@"device_list"]){
        printf("Creating persistent Connected list");
        deviceList = [NSMutableArray array];
        [deviceList addObject:valueID];
          }else{
              deviceList = [NSMutableArray arrayWithArray:[[NSUserDefaults standardUserDefaults] objectForKey:@"device_list"]];
              bool found = false;
              for (NSString* uuid in deviceList) {
                  if([uuid isEqualToString:valueID]){
                      found=YES;
                  }
              }
              if(found==NO){
    ...
    printf("Devices in persistent list");
    for (NSString* uuid in deviceList) {
        NSLog(@"Devices in persistent list %@",uuid);
    }

    
    [[NSUserDefaults standardUserDefaults] setObject:deviceList forKey:@"device_list"];
...
-(bool)allRegsiteredDevicesConnected{
    for(int i=([self.peripherals count]-1);i>=0;i--){
        CBPeripheral* periferal = [self.peripherals objectAtIndex:i];
        if(!periferal.isConnected){
        [self.peripherals removeObjectAtIndex:i];
            for(int f=0;f<[self.userPeripherals count];f++){
                if([[[self.userPeripherals objectAtIndex:f] valueForKey:@"identifier"] isEqualToString:[NSString stringWithFormat:@"%s",[self UUIDToString:periferal.UUID]]])
                    [self.userPeripherals removeObjectAtIndex:f];
            }
        }
    }

    for(int i=([self.userPeripherals count]-1);i>=0;i--){
        BOOL exist = false;
         for(int j=([self.peripherals count]-1);j>=0;j--){
             CBPeripheral* periferal = [self.peripherals objectAtIndex:j];
             if([[[self.userPeripherals objectAtIndex:i] valueForKey:@"identifier"] isEqualToString:[NSString stringWithFormat:@"%s",[self UUIDToString:periferal.UUID]]]){
                 exist = true;
             }
         }
        if(!exist){
    ...
    NSLog(@"======================================");
    NSLog(@"= Device connected: %lu",(unsigned long)self.peripherals.count);
    NSLog(@"======================================");
    
    NSArray* deviceList = [[NSUserDefaults standardUserDefaults] objectForKey:@"device_list"];
    if(deviceList){
        for(NSString* valueID in deviceList){
        bool exists = false;
            for(int i=0;i<self.peripherals.count;i++){
                NSString *oldvalueID = [NSString stringWithFormat:@"%s",[self UUIDToString:[[self.peripherals objectAtIndex:i] UUID]]];
                if([oldvalueID isEqualToString:valueID]){
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error{
    ...
    for(int i=([self.userPeripherals count]-1);i>=0;i--){
        printf("Diconnected device L8 number %i\r\n",i);
...
//        NSLog(@"Removed device from NSUSERDEFAULTS");
    ...
    if(self.delegate){
    [self.delegate disconnectL8];
    }
}

-(void)centralManager:(CBCentralManager *)central didFailToConnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error{
    printf("FAIL TO CONNECT to peripheral with  : %s removing from list\r\n",[self UUIDToString:peripheral.UUID]);
            ...
            if([self compareCBUUID:service.UUID UUID2:s.UUID])
            {
                if (i == (service.characteristics.count - 1))
                {
                  ...
                  //  [[self delegate] keyfobReady];
                    [[self delegate] keyfobReadyPeriferal:peripheral];
                }
            }
        }
    }
